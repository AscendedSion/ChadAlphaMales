...
void CRadar::Run()
{
	...
	//Outline
	g_Draw.OutlinedRect(m_nRadarX - m_nRadarSize - 1, m_nRadarY - m_nRadarSize - 20, m_nRadarCorrSize + 2, m_nRadarCorrSize + 22, { 0,0,0,255 });
	g_Draw.OutlinedRect(m_nRadarX - m_nRadarSize, m_nRadarY - m_nRadarSize - 19, m_nRadarCorrSize, m_nRadarCorrSize + 20, { 60, 60, 60, 255 });

	if (const auto& pLocal = g_EntityCache.m_pLocal)
		DrawPoints(pLocal);

	//Titlebar
	g_Draw.Rect(m_nRadarX - m_nRadarSize + 1, m_nRadarY - m_nRadarSize - 18, m_nRadarSize * 2 - 2, 18, { 20,20,20,255 });
	g_Draw.Rect(m_nRadarX - m_nRadarSize + 1, m_nRadarY - m_nRadarSize - 1, m_nRadarSize * 2 - 2, 1, Vars::Menu::Colors::WidgetActive);

	g_Draw.String(FONT_MENU, m_nRadarX - m_nRadarSize + 5, m_nRadarY - m_nRadarSize - 16, { 200,200,200,255 }, ALIGN_DEFAULT, _("Radar"));
...
void CRadar::DrawRadar()
{
	...
	if (g_Menu.menuOpen || g_Menu.m_flFadeElapsed < g_Menu.m_flFadeDuration)
	{
	...
	Color_t clrBack = { 10, 10, 10, static_cast<byte>(Vars::Radar::Main::BackAlpha.m_Var) };

	//g_Draw.GradientRect(m_nRadarX - m_nRadarSize, m_nRadarY - m_nRadarSize - 1, m_nRadarX + m_nRadarSize * 2, 10, { 0,0,0,0 }, Vars::Menu::Colors::WidgetActive, false);
...
void CRadar::DrawPoints(CBaseEntity* pLocal)
{
	...
	*/
	...
	{
		const auto Buildings = g_EntityCache.GetGroup(Vars::Radar::Players::IgnoreTeam.m_Var ? EGroupType::BUILDINGS_ENEMIES : EGroupType::BUILDINGS_ALL);
			...
			*/
			...
			if (GetDrawPosition(nX, nY, Player))
			{
				...
				*/
				...
				//PlayerInfo_t pi;
				//if (g_Interfaces.Engine->GetPlayerInfo(Player->GetIndex(), &pi))
					//g_Draw.String(FONT_MENU, nX - 3, nY + nSize, {255,255,255,255}, ALIGN_DEFAULT, pi.name);
...
void CRadar::DragRadar()
{
	...
	if ((mousex > (m_nRadarX - m_nRadarSize) &&
		...
		mousey >(m_nRadarY - m_nRadarSize) - 20 &&
